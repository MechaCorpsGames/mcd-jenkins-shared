"""The port sweep is EXECUTED, not just pattern-matched (mc-r15kh 3a).

test_mcd_drainer_handoff.py pins the SHAPE of the change: that
vars/mcdDrainerHandoff.groovy exists, that BOTH call sites in mcdServerPipeline use
...
This file runs the other half: test/groovy/mcd_drainer_handoff_behaviour.groovy
compiles vars/mcdDrainerHandoff.groovy, calls it, and RUNS the shell it returns under
...
a real gap: a skipped run of this file proves NOTHING about the handoff's
...
`docker` is a fake answering from a fixture, so this pins the handoff's logic and
...
_HARNESS = _REPO_ROOT / "test" / "groovy" / "mcd_drainer_handoff_behaviour.groovy"
_FAKE = _REPO_ROOT / "test" / "groovy" / "fake_docker_handoff.py"

_NO_RUNTIME = (
    "no Groovy runtime found, so the behaviour of mcdDrainerHandoff.groovy was NOT "
    "checked by this run: only its source shape was, by "
    "test_mcd_drainer_handoff.py. Put `groovy` on PATH, or set "
...
def test_the_harness_is_present_and_keeps_its_safety_property():
    ...
    assert _HARNESS.exists(), (
        f"{_HARNESS} is missing: the executed half of mc-r15kh 3a's coverage is gone"
    )
    assert _FAKE.exists(), f"{_FAKE} is missing: the harness cannot run without its fake docker"
    body = _HARNESS.read_text()
    assert "a YOUNG drainer from an earlier deploy is left alone" in body, (
        "the positive control is gone. Without it the whole suite passes for a handoff "
        "that stops every drainer it sees, which would kick exactly the matches this "
        "change protects"
    )
    assert "'renamed', 'updated', 'signalled'" in body, (
        "the ORDER assertion has been removed. Rename, then restart=no, then SIGUSR1 is "
        "the property; three things happening in any order is not the same claim"
    )


@pytest.mark.skipif(_groovy() is None, reason=_NO_RUNTIME)
def test_the_handoff_behaves_when_executed():
    ...
    assert result.returncode == 0, f"the executed handoff misbehaved:\n{output}"
    ...
    assert len(ran) >= 9, f"expected the full case set to run, saw {len(ran)}:\n{output}"
    # The handoff harness has no single invariant line; its equivalent guard is
    # that the order case ran, since that is the one whose absence would be
    # invisible in a pass/fail count.
    assert "renamed, de-policied and SIGUSR1d, in that order" in output, output
