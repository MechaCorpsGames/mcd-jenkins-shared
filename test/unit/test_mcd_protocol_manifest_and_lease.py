...
def test_the_deploy_creates_the_lease_directories() -> None:
    """A lease directory that does not exist is the failure we already had.

    The proxy creates what it can, but the base has to exist on the host for the
    bind mount to be useful, and a lease that silently no-ops is indistinguishable
    from the mc-2acos bug it replaces. The deploy creates it alongside the trees
    it already creates.
    """
    body = _stage_src("Deploy GameServer & TestClient")
    assert "mkdir -p" in body, "the deploy no longer creates its deploy-path directories"
    assert "/leases/versions" in body and "/leases/testclient-versions" in body, (
        "the deploy no longer creates the lease directories. Without them the "
        "bind mount has nothing behind it and the lease guard goes quiet again. "
        "See mc-2acos."
    )
