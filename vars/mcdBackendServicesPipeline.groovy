// MechaCorps Backend Services Pipeline - Shared Library
// Builds and deploys: CrashReporting (Log Bundler) + MCP Server
// Triggered when Src/CrashReporting/, Src/MCPServer/, or Src/Shared/ change.
...
def call(Map config) {
    ...
    pipeline {
        ...
        environment {
            ...
            ENV_FILE = "Src/.env.crash-reporting"
            COMPOSE_PROJECT = "src"
        }
        ...
        stages {
            ...
            stage('Build Docker Images') {
                steps {
                    sh """
                        cd Src
                        docker-compose -f docker-compose.crash-reporting.yml --env-file .env.crash-reporting -p ${COMPOSE_PROJECT} build crash-reporting mcp-server
            ...
            stage('Deploy') {
                steps {
                    sh """
                        cd Src
                        docker-compose -f docker-compose.crash-reporting.yml --env-file .env.crash-reporting -p ${COMPOSE_PROJECT} up -d --force-recreate crash-reporting mcp-server
                        echo "✓ Stack deployed"
                        ...
                        echo "Service status:"
                        docker-compose -f docker-compose.crash-reporting.yml --env-file .env.crash-reporting -p ${COMPOSE_PROJECT} ps
                    """
                }
            }
            ...
            stage('Health Check') {
                steps {
                    script {
                        ...
                        if (healthResult != 0) {
                            sh "cd Src && docker-compose -f docker-compose.crash-reporting.yml --env-file .env.crash-reporting -p ${COMPOSE_PROJECT} logs --tail=50 crash-reporting mcp-server"
